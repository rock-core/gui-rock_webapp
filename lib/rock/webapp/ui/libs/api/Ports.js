...
function constantPortUpdate(readurl, callback){
	createWebSocket(readurl, callback);
}

function stopConstantPortUpdate(readurl){
	var websock = portsockets[readurl];
	
	console.log(portsockets);
	console.log(websock);
...
function createWebSocket(url, callback){
	...
	var websocket = portsockets[url];
	if (!(typeof websocket == 'undefined')){
		websocket.close();
	}else{
		console.log ("didn't find websock" + url);
	}
	portsockets[url] = new WebSock(url,callback);
	console.log(portsockets);
	console.log ("added websock" + url);
};
