...
//		});
	...
	this.updatePortValue = function(taskname, portname){
		...
		getPort( this.url+"/tasks/"+taskname+"/ports/"+portname ,function(portinfo){
			...
			readPort(readurl, function(data){
				...
				var play = createButton("play","window['"+taskmgr.id+"'].constantUpdate(\""+id+"data\",\""+websock_url+"\")");
				var stop = createButton("stop","window['"+taskmgr.id+"'].stopConstantUpdate(\""+websock_url+"\")");
	...
	this.stopConstantUpdate = function(readurl){
		var taskmgr = this;
		stopConstantPortUpdate(readurl);
	}
	
	this.constantUpdate = function(targetid,readurl){
		var taskmgr = this;
		constantPortUpdate(readurl, function(content){
			taskmgr.updatePortContent(targetid,content);
		});
	}
