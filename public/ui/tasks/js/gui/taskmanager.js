...
function taskmanager(url,id, width, height){
	...
	JSON.load(url+"/tasks",function(data){
...
function TaskManager(url,id){
	...
	this.updateTaskState = function(taskname){
		...
		var tskmgr = this;
		Tasks.loadInfo(this.url+"/tasks/"+taskname,function(taskinfo){
	...
	this.addPorts = function(taskname){
		...
		Tasks.getInfo(this.url+"/tasks/"+taskname, function(data){
...
//		});
	...
	this.addPort = function(taskname, portinfo){
		...
		if (portinfo.direction == "output"){
			...
			var websock_url = porturl.replace(/http:\/\//,"ws://") + "/write";
			
			var taskmgr = this;
			//async call to the server, get the data type information
			Typelib.getTypeInfoOf(porturl,function(portinfo){
				...
				var form = generateForm(websock_url, portinfo, id);
	...
	this.updatePortValue = function(taskname, portname){
		var taskmgr = this;
		Ports.getPort( this.url+"/tasks/"+taskname+"/ports/"+portname ,function(portinfo){
		
			var url = taskmgr.url+"/tasks/"+taskname+"/ports/"+portinfo.name;
			var readurl = taskmgr.url+"/tasks/"+taskname+"/ports/"+portinfo.name+"/read?init=true&binary=true";

			Ports.read(readurl, function(data){
				...
				Typelib.getPortContentAsText(portinfo,data," ", function(text){
					if (text[0] == '{'){
	...
	this.stopConstantUpdate = function(taskname, portname){
		...
		var websock_url = readurl.replace(/http:\/\//,"ws://");
		WebSockets.delete(websock_url);
	}
	
	this.constantUpdate = function(taskname, portname){
		var taskmgr = this;
		
		Ports.getPort( taskmgr.url+"/tasks/"+taskname+"/ports/"+portname ,function(portinfo){
			...
			WebSockets.create(websock_url, function(content){
				var text = Typelib.getPortContentAsText(portinfo, content, "", function(text){
